...
const page = () => {
  ...
  // NEW: State for subscription and usage
  ...
  useEffect(() => {
    const fetchLimitData = async () => {
      ...
      try {
        ...
        console.log('Usage count:', count, 'Subscription:', subData);
      } catch (err) {
        console.error('Error fetching limit data:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const freeLimit = 5;
  const isLimitReached = !isPro && usageCount >= freeLimit;
  ...
  const handleDrop = (e) => {
    ...
    if (isLimitReached) {
      setError('Daily limit reached! Upgrade to Pro for unlimited compressions.');
      return;
    }
  ...
  const processFile = async (selectedFile) => {
    ...
    try {
      ...
      // Debug DB Save
      const dbSaveSuccess = response.headers.get('X-DB-Save-Success');
      const dbError = response.headers.get('X-DB-Error');
      console.log('DB Save Status:', dbSaveSuccess === 'true' ? 'SUCCESS' : 'FAILED');
      if (dbSaveSuccess !== 'true') {
        console.error('DB Save Error:', dbError);
        alert(`Warning: Compression worked but stats were not saved. Error: ${dbError}`);
      }
      ...
      <div style={{ padding: '20px' }}>
        ...
        {isLimitReached && !file && (
          <div style={{
            ...
            <h2 style={{ margin: '0 0 10px 0', color: '#856404' }}>🚫 Daily Limit Reached</h2>
            <p style={{ margin: '0 0 15px 0', fontSize: '16px' }}>
              You've used {usageCount}/{freeLimit} compressions today.
            </p>
            <button
            ...
            >
              ⭐ Upgrade to Pro for Unlimited Compressions
            </button>
          </div>
        )}
